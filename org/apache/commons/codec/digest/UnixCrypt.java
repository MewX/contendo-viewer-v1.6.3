...
/* 185 */     return crypt(original, (String)null);
/*     */   }
/*     */ 
/*     */ 
/*     */ 
...
/* 205 */     if (salt == null) {
/* 206 */       ThreadLocalRandom randomGenerator = ThreadLocalRandom.current();
/* 207 */       int numSaltChars = SALT_CHARS.length;
/*     */       
/* 209 */       salt = "" + SALT_CHARS[randomGenerator.nextInt(numSaltChars)] + SALT_CHARS[randomGenerator.nextInt(numSaltChars)];
/* 210 */     } else if (!salt.matches("^[./0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz]{2,}$")) {
/* 211 */       throw new IllegalArgumentException("Invalid salt value: " + salt);
/*     */     } 
/*     */     
/* 214 */     StringBuilder buffer = new StringBuilder("             ");
/* 215 */     char charZero = salt.charAt(0);
/* 216 */     char charOne = salt.charAt(1);
/* 217 */     buffer.setCharAt(0, charZero);
/* 218 */     buffer.setCharAt(1, charOne);
/* 219 */     int eSwap0 = CON_SALT[charZero];
/* 220 */     int eSwap1 = CON_SALT[charOne] << 4;
/* 221 */     byte[] key = new byte[8]; int i;
/* 222 */     for (i = 0; i < key.length; i++) {
/* 223 */       key[i] = 0;
/*     */     }
/*     */     
/* 226 */     for (i = 0; i < key.length && i < original.length; i++) {
/* 227 */       int iChar = original[i];
/* 228 */       key[i] = (byte)(iChar << 1);
/*     */     } 
/*     */     
/* 231 */     int[] schedule = desSetKey(key);
/* 232 */     int[] out = body(schedule, eSwap0, eSwap1);
/* 233 */     byte[] b = new byte[9];
...
/* 251 */           u = 128;
/*     */         } 
/* 253 */         buffer.setCharAt(j, (char)COV2CHAR[c]);
/*     */       } 
/*     */     } 
...
/* 307 */     left = t >>> 1 | t << 31;
/* 308 */     int[] results = new int[2];
/* 309 */     permOp(right, left, 1, 1431655765, results);
/* 310 */     right = results[0];
/* 311 */     left = results[1];
/* 312 */     permOp(left, right, 8, 16711935, results);
/* 313 */     left = results[0];
/* 314 */     right = results[1];
/* 315 */     permOp(right, left, 2, 858993459, results);
/* 316 */     right = results[0];
/* 317 */     left = results[1];
/* 318 */     permOp(left, right, 16, 65535, results);
/* 319 */     left = results[0];
/* 320 */     right = results[1];
/* 321 */     permOp(right, left, 4, 252645135, results);
/* 322 */     right = results[0];
/* 323 */     left = results[1];
/* 324 */     int[] out = new int[2];
/* 325 */     out[0] = left;
/* 326 */     out[1] = right;
/* 327 */     return out;
/*     */   }
...
/* 341 */     t = t >>> 4 | t << 28;
/* 342 */     el ^= SPTRANS[1][t & 0x3F] | SPTRANS[3][t >>> 8 & 0x3F] | SPTRANS[5][t >>> 16 & 0x3F] | SPTRANS[7][t >>> 24 & 0x3F] | SPTRANS[0][u & 0x3F] | SPTRANS[2][u >>> 8 & 0x3F] | SPTRANS[4][u >>> 16 & 0x3F] | SPTRANS[6][u >>> 24 & 0x3F];
/*     */ 
/*     */     
/* 345 */     return el;
/*     */   }
...
/* 349 */     int[] schedule = new int[32];
/* 350 */     int c = fourBytesToInt(key, 0);
/* 351 */     int d = fourBytesToInt(key, 4);
/* 352 */     int[] results = new int[2];
/* 353 */     permOp(d, c, 4, 252645135, results);
/* 354 */     d = results[0];
/* 355 */     c = results[1];
/* 356 */     c = hPermOp(c, -2, -859045888);
/* 357 */     d = hPermOp(d, -2, -859045888);
/* 358 */     permOp(d, c, 1, 1431655765, results);
/* 359 */     d = results[0];
/* 360 */     c = results[1];
/* 361 */     permOp(c, d, 8, 16711935, results);
/* 362 */     c = results[0];
/* 363 */     d = results[1];
/* 364 */     permOp(d, c, 1, 1431655765, results);
/* 365 */     d = results[0];
/* 366 */     c = results[1];
...
/* 378 */       c &= 0xFFFFFFF;
/* 379 */       d &= 0xFFFFFFF;
/* 380 */       int s = SKB[0][c & 0x3F] | SKB[1][c >>> 6 & 0x3 | c >>> 7 & 0x3C] | SKB[2][c >>> 13 & 0xF | c >>> 14 & 0x30] | SKB[3][c >>> 20 & 0x1 | c >>> 21 & 0x6 | c >>> 22 & 0x38];
/*     */ 
/*     */       
/* 383 */       int t = SKB[4][d & 0x3F] | SKB[5][d >>> 7 & 0x3 | d >>> 8 & 0x3C] | SKB[6][d >>> 15 & 0x3F] | SKB[7][d >>> 21 & 0xF | d >>> 22 & 0x30];
/*     */       
...
/* Location:              /mnt/r/ConTenDoViewer.jar!/org/apache/commons/codec/digest/UnixCrypt.class
